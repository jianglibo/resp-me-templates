...
retrieve_setting_value() {
	...
	local dot_path_in_settings=$1
	local file_to_read=${2:-"definition.json"}
	value=$(cat "$file_to_read" | jq .settings | jq -j | jq -r "$dot_path_in_settings")
	# Return the result
	echo "$value"
}

# should use in pipe style. like this: retrieve_setting_value '.connection' | json_to_kv_pairs
json_to_kv_pairs() {
	local content
	while IFS= read -r line; do
		content+="$line"$'\n'
	done
	env_vars=$(echo $content | tr '\n' ' ' | jq -r 'to_entries|map("\(.key)=\(.value|tostring)")|.[]' | tr '\n' ' ')
	echo $env_vars
}
