...
oneapi_deploy_id=$(echo $prevDeployResult | jq -r .oneapi_deploy_id)
...
# redeploy the oneapi-deploy definition
# this is event message output, not json.
echo "start deploy created definition. ${oneapi_deploy_id}"
deploy_deploy_definition "$oneapi_deploy_id"
